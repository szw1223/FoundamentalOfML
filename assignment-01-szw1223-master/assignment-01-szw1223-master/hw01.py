...
Author:  Zhewei Song
Date: 10th Sept 2019
Desc: Hello, this program can show three required plots. If you want to run the first plot, please # lines 124-137
      and 140-157. If you want to run the second plot, please # lines 118-121, 140-157 and 39. If you want to
      run the third plot, please # lines 118-121, 124-137 and 39.
...
'''This is the first plot requried'''
xrange = np.arange(-4.5, 4.5, 0.001)          # get equally spaced points in the xrange
X = np.array([xrange ** m for m in range(wLS.size)]).T
esty = X @ wLS  # compute the predicted value
esty0 = X @ wIRLS
plotData(x1, t1, xrange, esty, xrange, esty0, ['Training Data', 'LS Function', 'IRLS Function'])

'''This is the second plot requried'''
...
#
# for i in range(10):
#     X = np.array([xrange ** i for i in range(i + 1)]).T
#     YT_IRLS.append(float(np.linalg.norm(X@fitdataIRLS(x2, t2, i, 0.6) - t2)))
#
# for i in range(10):
#     X = np.array([xrange ** i for i in range(i + 1)]).T
#     YT_LS.append(float(np.linalg.norm(X@fitdataLS(x2, t2, i) - t2)))
# m = list(range(10))
# plotData(m, YT_LS, m, YT_IRLS, None, None, ['LS Function', 'IRLS Function'])

'''This is the third plot requried'''
# m = 10
# xrange = np.arange(-4.5, 4.495, 0.009)  # get equally spaced points in the xrange
# X = np.array([xrange ** m for m in range(m + 1)]).T
#
# YT_IRLS = []
# YT_LS = []
#
# for i in range(m):
#     a = (i + 1) * 0.1
#     YT_IRLS.append(float(np.linalg.norm(X@fitdataIRLS(x2, t2, m, a).T - t2)))
#
# for i in range(m):
#     YT_LS.append(float(np.linalg.norm(X@fitdataLS(x2, t2, m) - t2)))
# k = list(range(0, m))
# li = []
# for i in k:
#     li.append(i/10)
